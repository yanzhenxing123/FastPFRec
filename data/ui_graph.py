...
class Interaction(Data, Graph):
    def __init__(self, conf, training, test, valid):
        ...
        :param conf:
        :param training: [[user, item, weight]]
        :param test:
        :param valid:
        """
        Graph.__init__(self)
        Data.__init__(self, conf, training, test, valid)

        self.user = {}  # {user_id: user_id, "5224": 5224}
        self.item = {}  # {item_id: index, "2219": 0} len = 7597
        self.id2user = {}  # {user_id: user_id, "5224": 5224}
        self.id2item = {}  # {item_id: item_id, 0: '2219'} len =
        self.training_set_u = defaultdict(dict)  # {user_id: {item_id: 1.0}}
        self.training_set_i = defaultdict(dict)  # {item_id: {user_id: 1.0}}
        self.test_set = defaultdict(dict)  # {user_id: {item_id: 1.0, item_id: 1.0}}
        self.test_set_item = set()
        self.valid_set = defaultdict(dict)
        self.valid_set_item = set()
        self.__generate_set()
        self.user_num = len(self.training_set_u)  # yelp 5224
        self.item_num = len(self.training_set_i)  # yelp 7597
    ...
    def __create_sparse_bipartite_adjacency(self, self_connection=False):
        ...
        '''
        n_nodes = self.user_num + self.item_num  # 12821
        row_idx = [self.user[pair[0]] for pair in self.training_data]  # 112576

        col_idx = [self.item[pair[1]] for pair in self.training_data] # 112576
        user_np = np.array(row_idx)
        item_np = np.array(col_idx)
        ratings = np.ones_like(user_np, dtype=np.float32)
        tmp_adj = sp.csr_matrix(
            (ratings, (user_np, item_np + self.user_num)), shape=(n_nodes, n_nodes), dtype=np.float32
        )  # 右上角的矩阵
        adj_mat = tmp_adj + tmp_adj.T  # 加上item-user矩阵 = 总的矩阵
        if self_connection:
