...
class Recommender(object):
    def __init__(self, conf, training_set, test_set, valid_set, **kwargs):
        self.config = conf
        self.data = Data(self.config, training_set, test_set, valid_set)
        self.model_name = self.config['model.name']
        self.ranking = OptionConf(self.config['item.ranking'])  # -topN 10,20
        self.emb_size = int(self.config['embedding.size'])  # 64
        self.maxEpoch = int(self.config['num.max.epoch'])  # 500
        self.batch_size = int(self.config['batch_size'])  # 256
        self.lRate = float(self.config['learnRate'])  # 0.001
        self.reg = float(self.config['reg.lambda'])  # 0.0001
        self.output = OptionConf(self.config['output.setup'])  # -dir ./results...
    ...
    def initializing_log(self):
        ...
        for k in self.config.config:
            self.model_log.add(f"{k}={self.config[k]}")
