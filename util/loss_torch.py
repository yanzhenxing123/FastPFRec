...
def bpr_loss(user_emb, pos_item_emb, neg_item_emb):
    ...
    loss = -torch.log(10e-6 + torch.sigmoid(pos_score - neg_score))  # pos_score - neg_score
...
def InfoNCE(view1, view2, temperature, b_cos=True):
    ...
    """
    if b_cos:
        view1, view2 = F.normalize(view1, dim=1), F.normalize(view2, dim=1)  # L2归一化

    pos_score = (view1 * view2).sum(dim=-1)  # (256)
    pos_score = torch.exp(pos_score / temperature)
    ttl_score = torch.matmul(view1, view2.transpose(0, 1))  # (256, 64) * (64, 256) = (256, 256)交换两个维度，相当于逆置
    ttl_score = torch.exp(ttl_score / temperature).sum(dim=1)  # 计算所有样本之间的得分
