...
def next_batch_pairwise_fl_pse(data, batch_size, select_user_list, n_negs=1):
    ...
    for u_id in u_id_list:
        ...
        users = selected_df['user'].tolist()
        items = selected_df['item'].tolist()
        u_idx, i_idx, j_idx = [], [], []  # user_index, item_index, neg:_itemd_index
        item_list = list(data.item.keys())
        for i, user in enumerate(users):
            i_idx.append(data.item[items[i]])
            u_idx.append(data.user[user])
            for m in range(n_negs):  # 选取1个neg_item
                neg_item = choice(item_list)
                while neg_item in data.training_set_u[user]:
                    neg_item = choice(item_list)
                j_idx.append(data.item[neg_item])
        len_inter = len(u_idx)  # 8
        if len_inter == 1:
            yield u_idx, i_idx, j_idx
        else:
            num_pse = max(1, int(0.1 * len_inter))  # max(1, 0.1 * 8) 伪样本数
            random_numbers = sample(range(len_inter), len_inter - num_pse)  # 随机选得到 => [5, 4, 2, 1, 0, 6, 7]
